...
for i, k in enumerate(ks):
    ...
    ax.plot(np.arange(0, step+1, step), measured_beta, label=str(k)+r"$\times$"+str(k) + r"$ \ (\frac{dx_{max} = \ $" + str(cs[i]) + r"$)$")
    #ax.set_xlabel(r"$\beta^2 = \frac{||f_a||^2 }{||f||^2}$")
...
fig.savefig(f"distance_propagation_mod.pdf", format="pdf", dpi=fig.dpi, bbox_inches="tight")
